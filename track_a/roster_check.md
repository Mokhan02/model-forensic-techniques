...
**Status: 2/5 live-verified (Claude Sonnet 5, GPT-5.6 Sol — both 2026-09-09).
Remaining 3 (Gemini 3.1 Pro, Qwen 3.8, Muse Glimmer 30B) are still
...
| Qwen 3.8 | Local inference (Lambda), HF id `Qwen/Qwen3.8-27B-FP8` on H100/4090-class GPUs; **use `Qwen/Qwen3.8-27B` (bf16) on A100s** (see note) | **(a) raw, pending live confirmation** | No API summarization layer exists when the model is run locally — whatever tokens the model emits between `<think>` tags are what gets captured, same setup as the original Qwen3-14B work. Confidence here is **structural** (no intermediary model in the loop), not documentation-based, but still needs one real generation to confirm. Model card (fetched directly, 2026-09-09) says: `<think>...</think>` tags, thinking on by default, and — new detail worth checking at load time — the checkpoint has a **vision-encoder component** (native image/video), not a plain text-only causal LM; `live_check.py`'s `check_local()` tries `AutoModelForCausalLM` first, which is often still valid for text-only use but not guaranteed. **Live attempt 2026-09-09 on an A100 (compute capability 8.0):** the FP8 checkpoint failed to load — FP8 execution needs capability ≥8.9 (H100/4090), `transformers` detected the mismatch and tried its bf16-dequant fallback, which hit a real bug in this transformers version (`Fp8HfQuantizer.update_tp_plan()` — `AttributeError: 'NoneType' object has no attribute 'get'`, unrelated to the multimodal-loading question entirely). Not a finding about CoT-visibility — a hardware/quantizer-version note. Fix: load `Qwen/Qwen3.8-27B` (bf16, ~54GB) directly on A100s, skip the FP8 path. |
