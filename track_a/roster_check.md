...
| Qwen 3.8 | Local inference (Lambda), HF id `Qwen/Qwen3.8-27B-FP8` (FP8; bf16 is `Qwen/Qwen3.8-27B` — FP8 recommended, comfortably fits 80GB with KV-cache headroom, bf16's ~54GB leaves less) | **(a) raw, pending live confirmation** | No API summarization layer exists when the model is run locally — whatever tokens the model emits between `<think>` tags are what gets captured, same setup as the original Qwen3-14B work. Confidence here is **structural** (no intermediary model in the loop), not documentation-based, but still needs one real generation to confirm. Model card (fetched directly, 2026-09-09) says: `<think>...</think>` tags, thinking on by default, and — new detail worth checking at load time — the checkpoint has a **vision-encoder component** (native image/video), not a plain text-only causal LM; `live_check.py`'s `check_local()` tries `AutoModelForCausalLM` first, which is often still valid for text-only use but not guaranteed. |
